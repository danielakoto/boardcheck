...
export const useSounds = (soundOptions) => {
  const [sound, setSound] = useState(soundOptions[1]);
  ...
  useEffect(() => {
    const loadSound = async () => {
      try {
        ...
        if (!saved) return setSound(soundOptions[1]);

        // Re-match against soundOptions in case URLs changed
        setSound(soundOptions.find((s) => s.name === saved.name) ?? soundOptions[1]);
      } catch {
        setSound(soundOptions[1]);
      }
    };
